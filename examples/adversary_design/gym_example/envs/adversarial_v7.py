...
class Adversarial_v7(gym.Env):
    ...
    def __init__(self, n_clutter=0, size=5):
        ...
        # Add action for placing the goal.
        self.MAX_STEPS = 1
        self.size=size
        ...
        self.base_port_protagonist = 3000
        self.n_steps_trainer = 1000
        ...
        self.file_name = 'results_adv7_3.csv'
        
        with open(self.file_name, 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(["n_episode", "distance_to_goal", "goal_x", "goal_y", "reward_pro"])
    ...
    def step (self, action):
        if self.done:
          # should never reach this point
            ...
            print('Shortest path: ', self.distance_to_goal)
            
            # padd with zeros observation space (so before we did not consider walls)
            matrix = np.zeros((40, 40))
            matrix[self.agent_start_pos[0]*8 + 3, self.agent_start_pos[1]*8 + 3] = 2
            matrix[self.goal_pos[0]*8 + 3, self.goal_pos[1]*8 + 5] = 1
            ...
            list_of_elem = [self.n_episodes, self.distance_to_goal, self.goal_pos[0], self.goal_pos[1], reward_protagonist]
            ...
            self.reward = reward_protagonist
