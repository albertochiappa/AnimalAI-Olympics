...
class RandomMazeGenerator(MazeGenerator):
    ...
    def _generate_maze(self):
        ...
        self.complexity_new = int(self.complexity * (5 * (shape[0] + shape[1])))
        self.density_new     = int(self.density * ((shape[0] // 2) * (shape[1] // 2)))
        # Build actual maze
        ...
        Z[:, 0] = Z[:, -1] = 1
        # Make aisles
        for i in range(self.density_new):
            x, y = randint(0, shape[1]//2 + 1) * 2, randint(0, shape[0]//2 + 1) * 2
            Z[y, x] = 1
            for j in range(self.complexity_new):
                neighbours = []
